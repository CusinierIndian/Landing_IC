...
class DBOperations:
	...
	def susbscribe(self, subscriptionData):
		try:
			...
			return self.exceptionTransformer.transformException(Constants.INTEGRITY_ERROR_CODE, Constants.SUBSCRIBED_USER, Constants.STATUS_FAILED)
	...
	def insertUserInCareers(self, careers):
		try:
			...
			return self.exceptionTransformer.transformException(Constants.INTEGRITY_ERROR_CODE, Constants.EMAIL_OR_PHONENUMBER_EXISTS, Constants.STATUS_FAILED)
		except Exception as e:
	...
	def bookACook(self, cookBookingDetails):
		try:
			...
			return self.transformer.transformerBookACook(cookBookingDetails.query.filter_by(id=cookBookingDetails.id).first(), Constants.STATUS_SUCCESS,Constants.SUCCESS_CODE)
